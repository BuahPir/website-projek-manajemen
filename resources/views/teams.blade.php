<x-app-layout>
    ...
    <br>
    <div x-data="{ search: '' }" class="flex items-center rounded-md bg-slate-200 p-6 shadow-lg">
        <div class="container mx-auto">
            ...
            <div class="flex items-center justify-between mb-6 px-4">
                ...
                <input
                    ...
                    class="w-1/3 p-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
            </div>

            <!-- List of Teams -->
            <template x-for="team in {{ json_encode($allTeams) }}.filter(team => team.name.toLowerCase().includes(search.toLowerCase()))" :key="team.id">
                <div class="mb-6 px-4 bg-white rounded-lg shadow-md p-4">
                    <!-- Team Header with Name and Icon -->
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-people text-indigo-600" viewBox="0 0 16 16">
                                <path d="M15 14s1 0 1-1-1-4-5-4-5 3-5 4 1 1 1 1zm-7.978-1L7 12.996c.001-.264.167-1.03.76-1.72C8.312 10.629 9.282 10 11 10c1.717 0 2.687.63 3.24 1.276.593.69.758 1.457.76 1.72l-.008.002-.014.002zM11 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4m3-2a3 3 0 1 1-6 0 3 3 0 0 1 6 0M6.936 9.28a6 6 0 0 0-1.23-.247A7 7 0 0 0 5 9c-4 0-5 3-5 4q0 1 1 1h4.216A2.24 2.24 0 0 1 5 13c0-1.01.377-2.042 1.09-2.904.243-.294.526-.569.846-.816M4.92 10A5.5 5.5 0 0 0 4 13H1c0-.26.164-1.03.76-1.724.545-.636 1.492-1.256 3.16-1.275ZM1.5 5.5a3 3 0 1 1 6 0 3 3 0 0 1-6 0m3-2a2 2 0 1 0 0 4 2 2 0 0 0 0-4"/>
                            </svg>
                            <span class="font-semibold text-gray-700 text-lg" x-text="team.name"></span>
                        </div>
                        <form :action="'/teams/' + team.id" method="POST" class="delete-team-form">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-500 hover:text-red-700 delete-team-btn">
                    ...
                    <div class="flex items-center flex-wrap space-x-4 mt-4">
                        <template x-for="user in team.users" :key="user.id">
                            <div class="flex items-center space-x-2">
                                <div class="w-8 h-8 rounded-full overflow-hidden border-2 border-indigo-500">
                                    <img :src="user.picture ? '/pictures/' + user.picture : '/img/person-fill.svg'" :alt="user.name" class="w-8 h-8 object-cover">
                                </div>
